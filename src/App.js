...
function App() {
  ...
  return (
    <div className="App">
        In the year 
        <select style={{margin: '10px'}} onChange={handleDateChange}>
          {
            dates.map((year) => {
       ...
       the country
       <select style={{margin: '10px'}}  onChange={handleCountryChange}>
        {
        ...
        exported <span style={{margin: '10px'}} >{firearmsState || 'an unknown amount of'}</span> firearms.
    </div>
  );
}
